...
class BankProvider extends GetProviderImpl {
  @override
  Future<Either<Exception, TModel>> addEntity<TModel>(TModel entity) async {
    log("Adicionando nueva entidad de tipo:BankModel.");
    Map<String, dynamic> data = {};
    if (entity is BankModel) {
      data = entity.toJson();
  ...
  Future<Either<Exception, TList>> getAll<TList>() async {
    log("Cargando listado de bancos...");
    String url = '/nomenclator/v1.0.0/nomenclator/banks';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "accept": "application/json",
    ...
    final response = await processResponse(get(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> &&
            ...
            map.containsKey("banks")) {
          log('Lo q vieneneidnfoenofainfoiafidnswcofnawdef => $map[banks]');
          log('Lo q vieneneidnfoenofainfoiafidnswcofnawdef => ${BankModel.fromJson(map)}');

          return BankList.fromJson(map);
        } else if (map is Map<String, dynamic> && map.containsKey("fault")) {
          return Fault.fromJson(map['fault']);
        }
        return map;
  ...
  Future<Either<Exception, TModel>> getEntity<TModel>(dynamic id) async {
    log("Cargando entidad de tipo 'Bank' con identificador=$id");
    String url = '';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "Accept": "application/json",
  ...
  @override
  BankProvider setBaseUrl(String newBaseUrl) {
