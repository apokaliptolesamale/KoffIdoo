...
class CardGiftProvider extends GetProviderImpl {
  @override
    ...
    final response = await processResponse(get(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> &&
            ...
            map.containsKey("card_gift")) {
          log('cardGift => $map');
          //log('Lo q vienen coordenadas => ${OperationList.fromJson(map["operations"])}');
          /* if (map["card_gift"].toString().contains('[')) {
            log('Esto contiene parentesis es una lista ==> ${map["card_gift"].toString()}');
          } else {
          ...
          return ListCardGiftModel.fromJson(map);
        } else if (map is Map<String, dynamic> && map.containsKey("fault")) {
          return Fault.fromJson(map['fault']);
        }
        return map;
