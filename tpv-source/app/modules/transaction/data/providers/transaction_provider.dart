...
class TransactionProvider extends GetProviderImpl {
  Future<Either<Exception, TList>> addClientId<TList>() async {
    ...
    final response = await processResponse(post(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> && !map.containsKey("fault")) {
          return TransactionList.fromJson(map);
  ...
  Future<Either<Exception, TList>> getAll<TList>() async {
    ...
    Map<String, dynamic> filters = {"limit": 50, "offset": 0};
    final query = getShemaQueryFromMap(filters);
    final response = await processResponse(get(
      // "$url",
      "$url$query",
  ...
  @override
  TransactionProvider setBaseUrl(String newBaseUrl) {
