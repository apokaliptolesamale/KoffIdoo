...
class LocalBankDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalBankDataSourceImpl();

  @override
  BankProvider get getProvider => Get.find();
  ...
  BankModel getModelFromJson(String data) {
    return bankModelFromJson(data);
  }
  ...
  Future<BankList> requestBanks(String url) async {
    final content = _readFileAsync(url);
    return Future.value(bankListModelFromJson(content));
  }
...
class RemoteBankDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
