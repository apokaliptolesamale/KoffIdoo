...
import '../providers/clientinvoice_provider.dart';

/// Implementación de la interfaz de fuente de datos local: LocalDataSource
class LocalClientInvoiceDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalClientInvoiceDataSourceImpl();

  @override
  ClientInvoiceProvider get getProvider => Get.find();
  ...
  ClientInvoiceModel getModelFromJson(String data) {
    return clientinvoiceModelFromJson(data);
  }
  ...
  @override
  Future<ClientInvoiceModel> request(
  ...
  Future<ClientInvoiceList> requestClientInvoices(String url) async {
    final content = _readFileAsync(url);
    return Future.value(clientinvoiceListModelFromJson(content));
  }
...
class RemoteClientInvoiceDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
  ...
  Future<Either<Exception, TList>> addClientId<TList>(
    ...
    return getProvider.addClientId(params);
  }
  ...
  @override
  Future<Either<Exception, TModel>> getFactMensualEtecsa<TModel>(
      Map<String, dynamic> params) async {
    return getProvider.getFactMensualEtecsa(params);
  }
  ...
  Future<Either<Exception, TList>> listClientConfig<TList>(
      Map<String, dynamic> params) {
    return getProvider.listClientConfig(params);
  }
  
   Future<Either<Exception, TModel>> deleteClientId<TModel>(
      dynamic clientID) {
    return getProvider.deleteClientId(clientID);
  }
