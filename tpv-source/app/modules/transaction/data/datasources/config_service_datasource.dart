...
class LocalClientIdDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalClientIdDataSourceImpl();

  @override
  TransactionProvider get getProvider => Get.find();

  Future<Either<Exception, TList>> addClientId<TList>() {
    return getProvider.addClientId();
  }
  ...
  ClientServiceModel getModelFromJson(String data) {
    return ClientServiceModelFromJson(data);
  }
  ...
  @override
  Future<ClientServiceModel> request(
  ...
  Future<TransactionList> requestTransactions(String url) async {
    final content = _readFileAsync(url);
    log(" contenido leido.........$content");
    return Future.value(transactionListFromJson(content));
  }
...
class RemoteClientIdDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();

  RemoteClientIdDataSourceImpl();

  @override
  ClientServiceProvider get getProvider => Get.find();

  Future<Either<Exception, TModel>> addClientId<TModel>(dynamic entity) {
    return getProvider.addClientId(entity);
  }
  ...
  Future<Either<Exception, TList>> getClientIds<TList>(
      Map<String, dynamic> params) {
    return getProvider.getClientIds(params);
  }
  ...
  /* Future<Either<Exception, TList>> getListTransactions<TList>( 
    Map<String, dynamic> filters)  async {
    return getProvider.getListTransactions(filters);
  }*/
}
