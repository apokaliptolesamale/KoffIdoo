...
class LocalTransferDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalTransferDataSourceImpl();

  @override
  TransferProvider get getProvider => Get.find();
  ...
  TransferModel getModelFromJson(String data) {
    return transferModelFromJson(data);
  }
  ...
  Future<TransferList> requestTransfers(String url) async {
    final content = _readFileAsync(url);
    return Future.value(transferListModelFromJson(content));
  }
...
class RemoteTransferDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
  ...
  Future<Either<Exception, TModel>> transferToAccount<TModel>(
    ...
    return getProvider.transferToAccount(id, entity);
  }

  Future<Either<Exception, TModel>> transferToCard<TModel>(
      dynamic id, Map<String, dynamic> entity) {
    return getProvider.transferToCard(id, entity);
  }
