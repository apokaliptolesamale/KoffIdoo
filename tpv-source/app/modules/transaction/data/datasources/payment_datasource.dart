...
class LocalPaymentDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalPaymentDataSourceImpl();

  @override
  PaymentProvider get getProvider => Get.find();
  ...
  PaymentModel getModelFromJson(String data) {
    return paymentModelFromJson(data);
  }
  ...
  Future<PaymentList> requestPayments(String url) async {
    final content = _readFileAsync(url);
    return Future.value(paymentListModelFromJson(content));
  }
...
class RemotePaymentDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
