...
class PaymentRepositoryImpl<PaymentModelType extends PaymentModel>
    implements PaymentRepository<PaymentModelType> {
  late DataSource? datasource;

  PaymentRepositoryImpl();

  @override
  Future<Either<Failure, PaymentModelType>> add(entity) async {
    try {
      if (entity is Map) {
        entity = paymentModelFromJson(json.encode(entity));
      }
      ...
      buildDataSource(url);
      final payment = await datasource!.request(url, body, header);
  ...
  @override
  Future<Either<Failure, PaymentModelType>> delete(dynamic entityId) async {
    try {
      //TODO Asigne la url del API o recurso para adicionar
  ...
  @override
  Future<Either<Failure, EntityModelList<PaymentModelType>>> filter(
    ...
    try {
      ...
      buildDataSource(url);
      final payments = await datasource!.request(url, body, header);
  ...
  @override
  Future<Either<Failure, PaymentModelType>> get(dynamic entityId) async {
    try {
      //TODO Asigne la url del API o recurso para adicionar
  ...
  @override
  Future<Either<Failure, EntityModelList<PaymentModelType>>> getAll() async {
    try {
      //TODO Asigne la url del API o recurso para adicionar
      String url = "Url to get by field";
      //TODO construya la instancia personalizada de BodyRequest o utilice EmptyBodyRequest
  ...
  @override
  Future<Either<Failure, EntityModelList<PaymentModelType>>> getBy(
      Map params) async {
    try {
      final paymentList = await datasource!.getAll();

      FutureOr<Either<Failure, EntityModelList<PaymentModelType>>> process(
          EntityModelList<PaymentModelType> list) {
        EntityModelList<PaymentModelType> listToReturn =
            DefaultEntityModelList<PaymentModelType>();
      ...
      return paymentList.fold((l) => Left(ServerFailure(message: l.toString())),
          (r) => process(r.payment));
    } on ServerException {
      return Left(ServerFailure(message: 'Error !!!'));
    }
  ...
  @override
  Future<Either<Failure, PaymentModelType>> getPayment(dynamic id) => get(id);
  ...
  @override
  Future<Either<Failure, EntityModelList<PaymentModelType>>> paginate(
      int start, int limit, Map params) async {
    try {
      buildDataSource(PathsService.apiOperationHost);

      EntityModelList<PaymentModelType> operations = await datasource!
              .getProvider
              .paginate<EntityModelList<PaymentModelType>>(start, limit, params)
          as EntityModelList<PaymentModelType>;
  ...
  @override
  Future<Either<Failure, PaymentModelType>> update(
  ...
  DataSource _buildDataSource(String path) {
    ...
    if (path.startsWith(remoteRegExp)) {
      datasource = RemotePaymentDataSourceImpl();
    } else {
      datasource = LocalPaymentDataSourceImpl();
    }
