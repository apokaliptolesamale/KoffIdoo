...
class ProductProvider extends GetProviderImpl {
  ...
  Future<Either<Exception, TModel>> addEntity<TModel>(TModel entity) async {
    log("Adicionando nueva entidad de tipo:ProductModel.");
  ...
  Future<Either<Exception, TList>> addProductToCart<TList>(dynamic id) async {
  ...
  Future<Either<Exception, TModel>> getEntity<TModel>(dynamic id) async {
    log("Cargando entidad de tipo 'Product' con identificador=$id");
    String url = '';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "Accept": "application/json",
