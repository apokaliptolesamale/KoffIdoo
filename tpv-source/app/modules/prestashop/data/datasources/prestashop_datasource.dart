...
import '../providers/prestashop_provider.dart';

/// Implementación de la interfaz de fuente de datos local: LocalDataSource
class LocalPrestaShopDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalPrestaShopDataSourceImpl();

  @override
  PrestaShopProvider get getProvider => Get.find();
  ...
  @override
  Future<Either<Exception, TModel>> addEntity<TModel>(TModel card) async {
  ...
  PrestaShopModel getModelFromJson(String data) {
    return prestashopModelFromJson(data);
  }
  ...
  @override
  Future<PrestaShopModel> request(
  ...
  Future<PrestaShopList> requestPrestaShops(String url) async {
    final content = _readFileAsync(url);
    return Future.value(prestashopListModelFromJson(content));
  }
...
class RemotePrestaShopDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
