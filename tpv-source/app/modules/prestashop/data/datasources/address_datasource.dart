...
class LocalAddressDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalAddressDataSourceImpl();

  @override
  AddressProvider get getProvider => Get.find();
  ...
  AddressModel getModelFromJson(String data) {
    return addressModelFromJson(data);
  }
  ...
  Future<AddressList> requestAddresss(String url) async {
    final content = _readFileAsync(url);
    return Future.value(addressListModelFromJson(content));
  }
...
class RemoteAddressDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
