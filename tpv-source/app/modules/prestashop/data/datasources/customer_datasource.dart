...
class LocalCustomerDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalCustomerDataSourceImpl();

  @override
  CustomerProvider get getProvider => Get.find();
  ...
  CustomerModel getModelFromJson(String data) {
    return customerModelFromJson(data);
  }
  ...
  Future<CustomerList> requestCustomers(String url) async {
    final content = _readFileAsync(url);
    return Future.value(customerListModelFromJson(content));
  }
...
class RemoteCustomerDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
