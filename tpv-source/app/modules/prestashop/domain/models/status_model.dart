...
class StatusList<T extends StatusModel> implements EntityModelList<T> {
  ...
  factory StatusList.fromStringJson(String strJson) => StatusList(
        statuss: List<T>.from(json
            .decode(strJson)["statuss"]
            .map((x) => StatusModel.fromJson(x))),
      );
  factory StatusList.fromXml(String xml) {
    XmlDocument doc = XmlDocument.parse(xml);
    final list = doc.findAllElements("order_state");
    List<T> listStatus = [];
    for (var element in list) {
      listStatus.add(StatusModel.fromXml(element, (el) {
        return StatusModel(
          id: el.getElement("id")!.text,
          color: el.getElement("color")!.text,
          status: doc.findAllElements("name").first.firstChild!.text,
        );
      }) as T);
    }
    return StatusList(statuss: listStatus);
  }
...
class StatusModel extends Status implements EntityModel {
  ...
  StatusModel({
    ...
    required this.color,
  }) : super(id: id, color: color, status: status);
  ...
  Map<String, String> getColumnNames() {
    return {"id": "ID", "color": "Color", "status": "Estado"};
  }
