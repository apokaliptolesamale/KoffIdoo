...
class LocalConfigDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalConfigDataSourceImpl();

  @override
  ConfigProvider get getProvider => Get.find();
  ...
  ConfigModel getModelFromJson(String data) {
    return configModelFromJson(data);
  }
  ...
  Future<ConfigList> requestConfigs(String url) {
    final content = _readFileAsync(url);
    return Future.value(configListModelFromJson(content));
  }
...
class RemoteConfigDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
