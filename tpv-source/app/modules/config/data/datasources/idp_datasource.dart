...
import '../providers/idp_provider.dart';

/// Implementación de la interfaz de fuente de datos local: LocalDataSource
class LocalIdpDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalIdpDataSourceImpl();

  @override
  IdpProvider get getProvider => Get.find();
  ...
  IdpModel getModelFromJson(String data) {
    return idpModelFromJson(data);
  }
  ...
  Future<IdpList> requestIdps(String url) async {
    final content = _readFileAsync(url);
    return Future.value(idpListModelFromJson(content));
  }
...
class RemoteIdpDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
