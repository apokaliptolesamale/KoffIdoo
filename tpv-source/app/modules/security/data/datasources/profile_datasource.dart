...
class RemoteProfileDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();

  RemoteProfileDataSourceImpl();

  @override
  Future<ProfileModel> request(
  ...
  ProfileModel getModelFromJson(String data) {
    return profileModelFromJson(data);
  }

  @override
  ProfileProvider get getProvider => Get.find();
...
class LocalProfileDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();
  ...
  Future<ProfileList> requestProfiles(String url) async {
    final content = _readFileAsync(url);
    return Future.value(profileListModelFromJson(content));
  }
