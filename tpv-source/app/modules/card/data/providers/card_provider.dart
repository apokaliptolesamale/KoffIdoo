...
class CardProvider extends GetProviderImpl {
  @override
  Future<Either<Exception, TModel>> addEntity<TModel>(TModel entity) async {
    log("Adicionando nueva entidad de tipo:CardModel.");
    log('Este es el entity==> $entity');
    AddCardModel ad = entity as AddCardModel;
    log('Este es addddd==> ${ad.toJson()}');

    String url = '/cards/v1.0.0/cards';
    ...
    final response = await processResponse(post(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> && !map.containsKey("fault")
          ...
          log('Lo q viene del AddCard => $map');
          log('Lo q viene del AddCard.fromJson => ${CardModel.fromJson(map)}');
          ...
          return HttpServerException(
            fault: Fault(
                faultCode: mapFault.faultCode,
                type: mapFault.type,
                message: mapFault.message,
                description: mapFault
                    .description), /* Fault.fromJson({
              "code": mapFault.faultCode,
              "type": mapFault.type,
              "message": mapFault.message,
              "description":
                  "Error al adicionar tarjeta. ==> ${mapFault.description}"
            }), */
            //response: map['fault']['message'],
          );
        }
        return map;
      },
    ...
    return response.fold((l) {
      ...
      log('Esta es el mensaje del fault del api==> ${l.toString()}');
      return Left(l);
    }, (resp) {
      if (resp.statusCode == 200) {
        log('Este es el StatusText del ADDCARD=0> ${resp.statusText}');
        log('Este es el BODY del ADDCARD=0> ${resp.body}');
        return Right(resp.body);
      } else {
        log('Este es el StatusCODE del ADDCARD=0> ${resp.statusCode}');
        log('Este es el StatusText del ADDCARD=0> ${resp.statusText}');
        log('Este es el BODY del ADDCARD=0> ${resp.body["fault"]["message"]}');
        throw HttpServerException(
            ...
            fault: Fault.fromJson({
              ...
              "description": "Error al adicionar tarjeta."
            }));
      }
    });
  }
  ...
  Future<Either<Exception, TModel>> deleteEntity<TModel>(id) async {
    ...
    String url = '/cards/v1.0.0/cards/$id/delete';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "accept": "application/json",
        "Version": Constants.versionApk,
    ...
    final response = await processResponse(delete(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> &&
            ...
            map.containsKey("card")) {
          log('Lo q vieneneidnfoenofainfoiafidnswcofnawdef => ${map["card"]}');
          log('Lo q vieneneidnfoenofainfoiafidnswcofnawdef => ${CardModel.fromJson(map)}');

          return CardModel.fromJson(map['card']);
        } else if (map is Map<String, dynamic> && map.containsKey("fault")) {
      ...
      if (resp.statusCode == 200) {
        ...
        throw HttpServerException(
            ...
            fault: Fault.fromJson({
              "code": resp.statusCode,
              "type": "Error",
              "message": "Error al eliminar listado.",
              "description": "Error al eliminar listado."
            }));
      }
    });
  }
  ...
  Future<Either<Exception, TList>> getAll<TList>() async {
    log("Cargando listado de TArjetasasasdasdasdasd...");
    String url = '/cards/v1.0.0/cards';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "accept": "application/json",
  ...
  Future<Either<Exception, TModel>> getEntity<TModel>(dynamic id) async {
    log("Cargando entidad de tipo 'Card' con identificador=$id");
    String url = '';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "Accept": "application/json",
  ...
  Future<Either<Exception, TModel>> getFirstCard<TModel>() async {
    log("Cargando listado de TArjetasasasdasdasdasd...");
    String url = '/cards/v1.0.0/cards/first';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "accept": "application/json",
        "Version": Constants.versionApk,
  ...
  Future<Either<Exception, TModel>> getSetAsDefault<TModel>(
      dynamic entity) async {
    log("Cargando entidad de tipo 'Card' con identificador=$entity");
    String url = '/cards/v1.0.0/cards/default';
    /* late bool entityValue;
    if (entity == 'true') {
      entityValue = true;
    } else if (entity == 'false') {
      entityValue = false;
    }
    log('Este es el entityValue==> $entityValue'); */
    //Map<String, dynamic> data = {'funding_source_uuid': entityValue};
    /* if (entity is CardModel) {
      data = entity.toJson();
    ...
    final response = await processResponse(post(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> &&
            ...
            map.containsKey("card")) {
          log('Lo q vieneneidnfoenofainfoiafidnswcofnawdef => $map["card"]');
          log('Lo q vieneneidnfoenofainfoiafidnswcofnawdef => ${CardModel.fromJson(map["card"])}');
  ...
  @override
  CardProvider setBaseUrl(String newBaseUrl) {
