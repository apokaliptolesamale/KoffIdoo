...
class CordenateProvider extends GetProviderImpl {
  @override
  Future<Either<Exception, TModel>> addEntity<TModel>(TModel entity) async {
    log("Adicionando nueva entidad de tipo:CordenateModel.");
    Map<String, dynamic> data = {};
    if (entity is CordenateModel) {
      data = entity.toJson();
    ...
    final response = await processResponse(post(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> && !map.containsKey("fault")) {
          return CordenateModel.fromJson(map);
  ...
  Future<Either<Exception, TModel>> getEntity<TModel>(dynamic id) async {
    log("Cargando entidad de tipo 'Cordenate' con identificador=$id");
    String url = '/nomenclator/v1.0.0/nomenclator/coordinates';
    Map<String, String> headers = await HeaderRequestImpl(
      headers: {
        "accept": "application/json",
    ...
    final response = await processResponse(get(
      ...
      decoder: (map) {
        if (map is Map<String, dynamic> &&
          ...
          log('Lo q vienen cordenadas => ${map["coordinates"]}');
          log('Lo q vienen coordenadas => ${CordenateModel.fromJson(map["coordinates"])}');

          return CordenateModel.fromJson(map["coordinates"]);
        } else if (map is Map<String, dynamic> && map.containsKey("fault")) {
          return Fault.fromJson(map['fault']);
        }
        return map;
  ...
  @override
  CordenateProvider setBaseUrl(String newBaseUrl) {
