...
class LocalBalanceDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalBalanceDataSourceImpl();

  @override
  BalanceProvider get getProvider => Get.find();
  ...
  BalanceModel getModelFromJson(String data) {
    return balanceModelFromJson(data);
  }
  ...
  Future<BalanceList> requestBalances(String url) async {
    final content = _readFileAsync(url);
    return Future.value(balanceListModelFromJson(content));
  }
...
class RemoteBalanceDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();

  RemoteBalanceDataSourceImpl();
