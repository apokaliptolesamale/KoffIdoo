...
import '../providers/cordenate_provider.dart';

/// Implementación de la interfaz de fuente de datos local: LocalDataSource
class LocalCordenateDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalCordenateDataSourceImpl();

  @override
  CordenateProvider get getProvider => Get.find();
  ...
  CordenateModel getModelFromJson(String data) {
    return cordenateModelFromJson(data);
  }
  ...
  Future<CordenateList> requestCordenates(String url) async {
    final content = _readFileAsync(url);
    return Future.value(cordenateListModelFromJson(content));
  }
...
class RemoteCordenateDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();

  RemoteCordenateDataSourceImpl();
