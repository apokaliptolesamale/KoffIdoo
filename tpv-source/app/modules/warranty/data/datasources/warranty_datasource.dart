...
class LocalWarrantyDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalWarrantyDataSourceImpl();

  @override
  WarrantyProvider get getProvider => Get.find();
  ...
  WarrantyModel getModelFromJson(String data) {
    return warrantyModelFromJson(data);
  }
  ...
  Future<WarrantyList> requestWarrantys(String url) {
    final content = _readFileAsync(url);
    return Future.value(warrantyListModelFromJson(content));
  }
...
class RemoteWarrantyDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
