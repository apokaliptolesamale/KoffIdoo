...
import '/app/modules/product/domain/usecases/getby_product_usecase.dart';
...
class QrInfoOrderPage extends CustomView<ProductController> {
  late UseCase usecase;
  late CustomFutureBuilder fututeBuilder;
  late String idOrder;

  QrInfoOrderPage() {
    idOrder = Get.parameters["idOrder"] ?? "";
    usecase = GetProductByFieldUseCase<ProductModel>(Get.find())
        .setParamsFromMap({"idOrder": idOrder});
    Get.lazyPut(() => this);
  }
  @override
  ...
  AsyncWidgetBuilder<A> getBuilderByUseCase<A>(UseCase uc) {
    if (uc is GetProductByFieldUseCase<ProductModel>) {
      return getQrCodeByOrderBuilder<A>();
    }
    return getQrCodeByOrderBuilder();
  }
  ...
  AsyncWidgetBuilder<A> getQrCodeByOrderBuilder<A>() {
    var builder = (BuildContext context, AsyncSnapshot<A> snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) {
        return EmptyDataSearcherResult(
          child: Loading.fromText(key: key, text: "Cargando productos..."),
        );
      } else if (isDone(snapshot)) {
        final dart.Right resultData = snapshot.data as dart.Right;
        EntityModelList<ProductModel> productList = resultData.value;
        return QrInfoOrderView(
          idOrder: idOrder,
      ...
      return Loading.fromText(key: key, text: "Cargando productos...");
    };
    return builder;
  }
...
class QrInfoOrderPageImpl<T> extends CustomAppPageImpl<T> {
  @override
  ...
  static QrInfoOrderPageImpl builder({
    String name = "/warranty/qr-order-info",
    String keyMap = "QR_INFO",
    int index = -1,
    bool? popGesture,
    Map<String, String>? parameters,
    String? title,
  ...
  static GetPageBuilder getPageBuilder(String name, String keyMap) {
    ...
    return () => QrInfoOrderPage();
  }
}

class QrInfoOrderView extends StatefulWidget {
  final EntityModelList<ProductModel> productList;
  ...
  @override
  _QrInfoOrderViewState createState() => _QrInfoOrderViewState();
}

class _QrInfoOrderViewState extends State<QrInfoOrderView> {
  int index = 1;
  ...
  Widget build(BuildContext context) {
    ...
    String qrUrlService =
        "${PathsService.qrUrlService}dataUrl=${PathsService.merchantUrlService}dispatch-order/${widget.idOrder}";
    ...
    final listView = ListView.separated(
      ...
      itemCount: widget.productList.getTotal,
      itemBuilder: (_, i) {
        final product = widget.productList.getList().elementAt(i);
        ...
        return ElevatedButton(
          ...
          onPressed: () {
            Get.toNamed(Routes.getInstance.getPath("PRODUCT_INFO"),
                parameters: {"idProduct": product.idProduct});
          },
          child: Container(
  ...
  void initState() {
    ...
    searchInput = SearchText(
      ...
      decoration: InputDecoration(
        ...
        hintText: "Buscar información del producto...",
        //prefixIcon: icon,
