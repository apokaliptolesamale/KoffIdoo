...
class LocalNotifyDataSourceImpl implements LocalDataSource {
  final http.Client client = Get.find();

  LocalNotifyDataSourceImpl();

  @override
  NotifyProvider get getProvider => Get.find();
  ...
  NotifyModel getModelFromJson(String data) {
    return notifyModelFromJson(data);
  }
  ...
  Future<NotifyList> requestNotifys(String url) async {
    final content = _readFileAsync(url);
    return Future.value(notifyListModelFromJson(content));
  }
...
class RemoteNotifyDataSourceImpl implements RemoteDataSource {
  final http.Client client = Get.find();
